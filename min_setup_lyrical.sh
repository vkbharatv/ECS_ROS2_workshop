...
if [[ "${ID:-}" != "ubuntu" || "${VERSION_ID:-}" != "26.04" ]]; then
    echo "This script targets Ubuntu 26.04 only."
...
echo "[4/7] Install ROS 2 Lyrical + RViz + Gazebo + MoveIt"
sudo apt install -y \
    ros-lyrical-desktop \
    ros-lyrical-rviz2 \
    ros-lyrical-ros-gz \
...
if ! grep -Fxq "source /opt/ros/lyrical/setup.bash" "$HOME/.bashrc"; then
